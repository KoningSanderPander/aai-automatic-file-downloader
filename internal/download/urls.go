package download

var (
	files = []Download{
		{
			Name: "group-photo-1.png",
			Url:  "https://i0.wp.com/digital-photography-school.com/wp-content/uploads/2021/03/how-to-take-group-photos-8.jpg?resize=1500%2C1000&ssl=1",
		},
		{
			Name: "group-photo-2.png",
			Url:  "https://media.istockphoto.com/photos/group-portrait-of-a-creative-business-team-standing-outdoors-three-picture-id1146473249?k=20&m=1146473249&s=612x612&w=0&h=9Ki3nKs4Su-_YRMc6__iuWnHLhpp58ULOsz4l9PT6tw=",
		},
	}
)
